...
def extract_education(text):  # <-- SINGLE CORRECT DEFINITION
    ...
    if not education:
        fallback_pattern = r'(?:Education|Academic\s+Background)[\s:-]+((?:.|\n)+?)(?=\n\s*\n|Work\s+Experience|Employment|Skills|$)'
        education_section = re.search(fallback_pattern, text, re.IGNORECASE)
